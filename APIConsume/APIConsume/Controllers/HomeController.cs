...
namespace APIConsume.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> GetReservation(int id)
        {
            ...
            {
                using (var response = await httpClient.GetAsync("http://localhost:5190/api/reservation/" + id))
                {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                using (
                    var response = await httpClient.GetAsync("http://localhost:5190/api/reservation")
                    )
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> AddReservation(Reservation reservation)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (
                var response = await httpClient.PostAsync("http://localhost:5190/api/Reservation/", content))
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateReservation(int id)
        {
            ...
            {
                using (var response =
                    await httpClient.GetAsync("http://localhost:5190/api/Reservation/" + id))
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
        ...
        public async Task<IActionResult> UpdateReservation(Reservation reservation)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PutAsync("http://localhost:5190/api/Reservation/", content))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        [HttpPost]
        public async Task<IActionResult> DeleteReservation(int Reservation)
        {
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.DeleteAsync("http://localhost:5190/api/Reservation/" + Reservation))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                }
            }
