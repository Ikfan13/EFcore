...
namespace SecurityDemo.Controllers
{
    public class CallAPIController : Controller
    {
        ...
        public async Task<IActionResult> FlightReservation()
        {
            ...
            //    using (var response = await httpClient.GetAsync("https://localhost:5168/Reservation")) {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.GetAsync("http://localhost:5168/Reservation"))
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
